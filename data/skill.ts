/**
 * Skills API Data
 *
 * @author Andy Chen
 */

const data = {
  heading: "THE SKILLS I GAINED FROM THE PAST DECADE",
  skills: {
    languages:
      "JavaScript( 5 yrs+ ), Java( 3 yrs+ ), C#( 3 yrs+ ), Python, NodeJS, TypeScript, HTML, CSS, SQL",
    framework:
      "ReactJS, ReactNative, VueJS, RequireJS, Zepto, jQuery, Mootools, Express, Java Servlet, Velocity, Asp.net, MVC, MVVM, WPF, WCF",
    databases: "PostgreSQL, MySQL, SqlServer, MongoDB",
    tools:
      "Linux( 5 yrs+ ), GitHub, GitLab, Git( 5 yrs+ ), Docker, Nginx, Amazon AWS, Alibaba Cloud, Webpack, Gulp, Gradle, Markdown",
  },
  others: {
    english:
      "Good English Capabilities in Listening, Reading, Writing, and Speaking.",
  },
};

export default data;
